...
func NewDockerSandbox(cfg *SandboxConfig) (*DockerSandbox, error) {
	...
	return &DockerSandbox{cli: cli, config: cfg}, nil
}

func (d *DockerSandbox) ID() string { return d.containerID }
...
func (d *DockerSandbox) Start(ctx context.Context, image string) error {
	...
	hostConfig := &container.HostConfig{
		Resources:   container.Resources{Memory: d.config.MemoryLimit},
...
func (d *DockerSandbox) GetLogs(ctx context.Context) (string, error) {
	...
	out, err := d.cli.ContainerLogs(ctx, d.containerID, container.LogsOptions{ShowStdout: true, ShowStderr: true})
	if err != nil {
		return "", err
	}
	defer out.Close()
