...
export interface PackageEntry {
  ...
  line: number;
  /** Zero-based character offset of the package name start */
  nameStart: number;
  /** Zero-based character offset of the package name end */
  nameEnd: number;
...
export function detectRegistry(document: vscode.TextDocument): Registry | null {
...
export function parseManifest(document: vscode.TextDocument): PackageEntry[] {
...
function parsePackageJson(doc: vscode.TextDocument, text: string): PackageEntry[] {
  ...
  const depSections: Array<{ key: string; isDev: boolean }> = [
  ...
  for (const { key, isDev } of depSections) {
    ...
    for (const [pkgName, version] of Object.entries(section as Record<string, string>)) {
...
function parseGoMod(doc: vscode.TextDocument, text: string): PackageEntry[] {
  const entries: PackageEntry[] = [];
...
function parseRequirementsTxt(doc: vscode.TextDocument, text: string): PackageEntry[] {
  const entries: PackageEntry[] = [];
  const lines = text.split('\n');
...
function parseCargoToml(doc: vscode.TextDocument, text: string): PackageEntry[] {
  const entries: PackageEntry[] = [];
  const lines = text.split('\n');
...
function parseComposerJson(doc: vscode.TextDocument, text: string): PackageEntry[] {
  const entries: PackageEntry[] = [];
  ...
  const sections: Array<{ key: string; isDev: boolean }> = [
...
function parseGemfile(doc: vscode.TextDocument, text: string): PackageEntry[] {
  const entries: PackageEntry[] = [];
  const lines = text.split('\n');
...
// ---------------------------------------------------------------------------
// Utility: find a string token in a document and return its position
// ---------------------------------------------------------------------------
